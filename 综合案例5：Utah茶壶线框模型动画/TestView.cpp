...
CTestView::CTestView()
{
	...
	bPlay = FALSE;
	int n = 3;//递归次数
	teapot.SetRecursiveNumber(n);//设置递归深度
	teapot.ReadVertex();//读入顶点
	teapot.ReadPatch();//读入曲面	
	transform.SetMatrix(teapot.Ver, 306);//设置顶点变矩阵
	double nScalar = 150;//比例
	transform.Scale(nScalar, nScalar, nScalar);//缩放
	transform.Translate(0, -200, 0);//平移
