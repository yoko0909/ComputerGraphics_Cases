...
void CBicubicBezierPatch::ReadTexture(CTexture* pTexture)//读入纹理
{
	this->pTexture = pTexture;
}

void CBicubicBezierPatch::SetScene(CLighting *pLight, CMaterial *pMaterial)//设置光源及材质
{
	this->pLight = pLight;
...
void CBicubicBezierPatch::DrawCurvedPatch(CDC* pDC, CZBuffer*pZBuffer, int nPatch)
{
	...
	Recursion(pDC, pZBuffer, ReNumber, Rect, nPatch);//递归函数
}
...
void CBicubicBezierPatch::Recursion(CDC* pDC, CZBuffer* pZBuffer, int ReNumber, CT2* pRect, int nPatch)
{
	if(ReNumber == 0)
	{
		...
		DrawFacet(pDC, pZBuffer, P, N, T);//绘制Facet
		return;
	}
	else
	{
...
void CBicubicBezierPatch::Tessellation(CT2* pRect, CP3* P, CVector3* N, CT2* T, int nPatch)
{
	...
	CP3 VC[4][4];//曲线计算用控制点数组
	for (int i = 0; i < 4; i++)
		for (int j = 0; j < 4; j++)
			VC[i][j] = V3[i * 4 + j];
	LeftMultiplyMatrix(M, VC);//系数矩阵左乘三维点矩阵
	TransposeMatrix(M);//计算转置矩阵
	RightMultiplyMatrix(VC, M);//系数矩阵右乘三维点矩阵
	...
	for (int i = 0;i < 4;i++)
	{
		...
		N[i] = CrossProduct(CVector3(P0), CVector3(P1)).Normalize();//顶点法向量
	...
	if (fabs(P[0].x - P[1].x) < 1e-6)//处理特殊区域法向
